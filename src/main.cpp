...
void setup() {
  // read caps level ASAP
  volts_perc = uint8_t(( analogRead( ADC_VOLT_PIN) / 8.76) + 0.5) - 200;   // range: 2.5V - 5V => 0 - 200
...
void loop() {
  ...
  sample_count++;
  if( volts_perc >= VOLTS_PERC(4.9) || // above this voltage send always
  ...
  if( volts_perc < VOLTS_PERC(3.8)){ // below 3.8V
    samples_to_send = 16;
  } else {
    samples_to_send = 8;
  }
  // if( volts_perc < VOLTS_PERC(3.5)){  // below 3.5V
